...
public class FuzzyAirconController extends PApplet{
    ...
    public void createFuzzyEngine(){
        ...
        inputVariable1.addTerm(new Trapezoid("verycold", 0.00, 0.00, 5.00, 10.00));
        inputVariable1.addTerm(new Trapezoid("cold", 5.00, 10.00, 15.00, 20.00));
        inputVariable1.addTerm(new Triangle("warm", 15.00, 20.00, 25.00));
        inputVariable1.addTerm(new Trapezoid("hot", 20.00, 25.00, 30.00, 35.00));
        inputVariable1.addTerm(new Trapezoid("veryhot", 30.00, 35.00, 40.00, 40.00));
        ...
        inputVariable2.addTerm(new Trapezoid("verycold", 0.00, 0.00, 5.00, 10.00));
        inputVariable2.addTerm(new Trapezoid("cold", 5.00, 10.00, 15.00, 20.00));
        inputVariable2.addTerm(new Triangle("warm", 15.00, 20.00, 25.00));
        inputVariable2.addTerm(new Trapezoid("hot", 20.00, 25.00, 30.00, 35.00));
        inputVariable2.addTerm(new Trapezoid("veryhot", 30.00, 35.00, 40.00, 40.00));
        ...
        outputVariable.addTerm(new Trapezoid("coolhigh", -10, -10, -7.5, -5.0));
        outputVariable.addTerm(new Trapezoid("cool", -7.5, -5.0, -2.5, 0));
        outputVariable.addTerm(new Triangle("nochange", -2.5, 0, 2.5));
        outputVariable.addTerm(new Trapezoid("heat", 0, 2.5, 5, 7.5));
        outputVariable.addTerm(new Trapezoid("heathigh", 5.0, 7.5, 10.0, 10.0));
...
//        ruleBlock.addRule(Rule.parse("if (room is verycold) then command is heathigh", engine));
//        ruleBlock.addRule(Rule.parse("if (room is veryhot) then command is coolhigh", engine));
//        ruleBlock.addRule(Rule.parse("if (room is verycold or room is cold) and (target is warm) then command is heat", engine));
//        ruleBlock.addRule(Rule.parse("if (room is veryhot or room is hot) and (target is warm) then command is cool", engine));
    
        //verycold
        ruleBlock.addRule(Rule.parse("if (room is verycold) and (target is hot) then command is heathigh", engine));
        ruleBlock.addRule(Rule.parse("if (room is verycold) and (target is veryhot) then command is heathigh", engine));
        ruleBlock.addRule(Rule.parse("if (room is verycold) and (target is warm) then command is heathigh", engine));
        ruleBlock.addRule(Rule.parse("if (room is verycold) and (target is verycold) then command is nochange", engine));
        ruleBlock.addRule(Rule.parse("if (room is verycold) and (target is cold) then command is heat", engine));

        //cold
        ruleBlock.addRule(Rule.parse("if (room is cold) and (target is verycold) then command is cool", engine));
        ruleBlock.addRule(Rule.parse("if (room is cold) and (target is cold) then command is nochange", engine));
        ruleBlock.addRule(Rule.parse("if (room is cold) and (target is warm) then command is heat", engine));
        ruleBlock.addRule(Rule.parse("if (room is cold) and (target is hot) then command is heathigh", engine));
        ruleBlock.addRule(Rule.parse("if (room is cold) and (target is veryhot) then command is heathigh", engine));

        ruleBlock.addRule(Rule.parse("if (room is warm) and (target is verycold) then command is coolhigh", engine));
        ruleBlock.addRule(Rule.parse("if (room is warm) and (target is cold) then command is cool", engine));
        ruleBlock.addRule(Rule.parse("if (room is warm) and (target is warm) then command is nochange", engine));
        ruleBlock.addRule(Rule.parse("if (room is warm) and (target is hot) then command is heat", engine));
        ruleBlock.addRule(Rule.parse("if (room is warm) and (target is veryhot) then command is heathigh", engine));

        ruleBlock.addRule(Rule.parse("if (room is hot) and (target is verycold) then command is coolhigh", engine));
        ruleBlock.addRule(Rule.parse("if (room is hot) and (target is cold) then command is coolhigh", engine));
        ruleBlock.addRule(Rule.parse("if (room is hot) and (target is warm) then command is cool", engine));
        ruleBlock.addRule(Rule.parse("if (room is hot) and (target is hot) then command is nochange", engine));
        ruleBlock.addRule(Rule.parse("if (room is hot) and (target is veryhot) then command is heat", engine));
       
        ruleBlock.addRule(Rule.parse("if (room is veryhot) and (target is verycold) then command is coolhigh", engine));
        ruleBlock.addRule(Rule.parse("if (room is veryhot) and (target is cold) then command is coolhigh", engine));
        ruleBlock.addRule(Rule.parse("if (room is veryhot) and (target is warm) then command is coolhigh", engine));
        ruleBlock.addRule(Rule.parse("if (room is veryhot) and (target is hot) then command is cool", engine));
        ruleBlock.addRule(Rule.parse("if (room is veryhot) and (target is veryhot) then command is nochange", engine));
